...
| `should`, `would`, `may`, `might`, `could` | Unapproved modal (Rule 3.2) | See `references/rules.md`. |
...
| `e.g.`, `i.e.`, `etc.` | Latin abbreviation (GR-6) | Use English words or rewrite the sentence. |
...
For each violation give: the rule number, the offending text, and a compliant rewrite. Cite only rule numbers that appear in `references/rules.md`.
