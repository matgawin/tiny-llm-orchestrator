...
func initializeTempRunLayout(tempDir, runID string) error {
	for _, dir := range artifactDirs() {
		if err := os.Mkdir(filepath.Join(tempDir, dir), runDirPerm); err != nil {
			return fmt.Errorf("create run %q artifact directory %s: %w", runID, dir, err)
		}
	}

	if err := os.Mkdir(filepath.Join(tempDir, configDirName), runDirPerm); err != nil {
...
func createTempRunLock(tempDir, runID string) error {
	...
	lockFile, err := os.OpenFile(filepath.Join(tempDir, ".lock"), os.O_CREATE|os.O_RDWR|syscall.O_NOFOLLOW, runFilePerm) // #nosec G304,G703 -- lock path is scoped to a newly created temporary run directory.
	if err != nil {
...
func reserveRunDir(runDir string) (*runDirReservation, error) {
	if err := os.Mkdir(runDir, runDirPerm); err != nil {
		return nil, fmt.Errorf("reserve run dir: %w", err)
	}

	reservation := &runDirReservation{path: runDir}

	file, err := os.OpenFile(filepath.Join(runDir, ".lock"), os.O_CREATE|os.O_RDWR|syscall.O_NOFOLLOW, runFilePerm) // #nosec G304,G703 -- lock path is scoped to an atomically reserved run directory.
	if err != nil {
		reservation.cleanup()
		return nil, fmt.Errorf("reserve run dir: %w", err)
	}
