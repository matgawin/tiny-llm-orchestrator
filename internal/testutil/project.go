...
const (
	testProjectDirPerm  = 0o750
	testProjectFilePerm = 0o600
)
...
func WriteProject(t *testing.T, root string, opts ProjectOptions) {
	...
	if err := os.MkdirAll(filepath.Join(orcDir, "workflows"), testProjectDirPerm); err != nil {
		t.Fatalf("create workflows dir: %v", err)
	}

	if err := os.MkdirAll(filepath.Join(orcDir, "agents"), testProjectDirPerm); err != nil {
		t.Fatalf("create agents dir: %v", err)
	}

	if err := os.MkdirAll(filepath.Join(orcDir, "runtimes"), testProjectDirPerm); err != nil {
...
func writeFile(t *testing.T, path, content string) {
	...
	if err := os.WriteFile(path, []byte(content), testProjectFilePerm); err != nil {
