...
func TestRunYesCreatesValidScaffoldAndIgnoreEntry(t *testing.T) {
	...
	assertFileContains(t, filepath.Join(root, ".orc", "config.yaml"), "runtimes:\n  codex: runtimes/codex.yaml\n")
	assertFileContains(t, filepath.Join(root, ".orc", "runtimes", "codex.yaml"), "  args: [exec, --skip-git-repo-check, \"-\"]\n")
	assertFileContains(t, filepath.Join(root, ".orc", "workflows", "implementation.yaml"), "  runtime: codex\n")
